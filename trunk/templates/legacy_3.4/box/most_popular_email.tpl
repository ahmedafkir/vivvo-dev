			...
			<div class="box_title">
				<ul id="most_popular_commented_tabs" class="tabs">
					<vte:if test="{VIVVO_MODULES_MOST_POPULAR}">
						<li><a href="#box_most_popular"><span><vte:value select="{LNG_MOST_POPULAR}" /></span></a></li>
					</vte:if>
					<vte:if test="{VIVVO_MODULES_MOST_EMAILED}">
						<li><a href="#box_most_emailed"><span><vte:value select="{LNG_MOST_EMAILED}" /></span></a></li>
					</vte:if>
				</ul>
			</div>
		</div>
