<vte:if test="{VIVVO_COMMENTS_ENABLE}">
	<vte:box module="box_comments">
		<vte:params>
			<vte:param name="search_user_id" value="{CURRENT_AUTHOR.get_id}" />
			<vte:param name="search_limit" value="5" />
			<vte:param name="search_sort_by" value="created" />
			<vte:param name="search_order" value="descending" />
			<vte:param name="cache" value="1" />
		</vte:params>
		<vte:template>
			<div id="author_latest_comments" class="box">
				<div class="box_title_holder"><div class="box_title"><vte:value select="{LNG_LATEST_COMMENTS}" /></div></div>
				<div class="box_body">
					<div class="box_content">
						<vte:if test="{comment_list}">
                            <vte:foreach item = "comment" from="{comment_list}">
                                <div class="single_comment">
                                    <div class="comment_header">
                                        <img src="{comment.get_gravatar|24}" alt="{comment.get_author}" width="24" height="24" />
										<a href="{comment.get_article_href}"><vte:value select="{comment.get_article_title}" /></a>
                                        <vte:value select="{LNG_ARTICLE_COMMENTS_POSTED_ON}" />
                                        <vte:value select="{comment.get_create_dt}" />
                                    </div>
                                    <div class="comment_body">
                                        <a href="{comment.get_article_href}"><vte:value select="{comment.get_summary}" /></a>
                                    </div>
                                </div>
                            </vte:foreach>
                        </vte:if>
					</div>
				</div>
			</div>
		</vte:template>
	</vte:box>
</vte:if>