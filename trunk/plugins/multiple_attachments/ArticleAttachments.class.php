 ...
 */
class ArticleAttachments extends vivvo_post_object {
	 ...
	 * @var   string
	 */
	var $_sql_table = 'article_attachments';
	...
	function get_filename(){
		return basename(VIVVO_FS_ROOT . VIVVO_FS_FILES_DIR . $this->real_path);
	}
	...
	function get_extension(){
		$info = pathinfo(VIVVO_FS_ROOT . VIVVO_FS_FILES_DIR . $this->real_path);
		return $info['extension'];
	}
}

/**
 * ArticleAttachments list
 *
 * @license		http://www.vivvo.net/license.php
 * @copyright	SpoonLabs
 * @author		Slobodan Utvic <utvara@spoonlabs.com>
 ...
 * @version		Vivvo Lite - Generic Database Engine
 */
class ArticleAttachments_list extends vivvo_db_paged_list {
	var $_sql_table = 'article_attachments';
	var $post_object_type = 'ArticleAttachments';

	function _default_query(){
		$this->_query->set_from(
								VIVVO_DB_PREFIX . 'article_attachments ');
	...
	function &get_attachments_by_article_id($article_id){
		$article_id = (int) $article_id;
		$this->_default_query();
		$this->add_filter('article_id' , $article_id);
		$this->_query->add_order('order_number ASC');
		$this->set_list();
		if (!empty($this->list)){
			return  $this->list;
		}else{
	...
	function &get_attachment_by_id($image_id){
		$image_id = (int) $image_id;
		$this->_default_query();
		$this->add_filter('id' , $image_id);
		$this->set_list();
		if (!empty($this->list)){
