...
/* vivvo_cache support begin */
            $expires = time() + 10;
/* vivvo_cache support end */
            if (($doNotTestCacheValidity) || (is_null($this->_refreshTime))) {
                if (file_exists($this->_file)) {
/* vivvo_cache support begin */
                    $data = $this->_read($expires);
    #                $data = $this->_read();
/* vivvo_cache support end */
                }
            } else {
...
/* vivvo_cache support end */
        ...
        while ($file = readdir($dh)) {
            if (($file != '.') && ($file != '..')) {
/* vivvo_cache support begin */
                if (substr($file, 0, $motif_len) == $motif) {
...
/* vivvo_cache support begin */
    /**
     * Checks if cache file is expired or not
     *
     * @param   string  $cache_file
     * @return  bool
     */
    function _valid($cache_file = null) {
        $fp = @fopen($cache_file ? $cache_file : $this->_file, 'rb');
        $expires = hexdec(@fread($fp, 16));
        @fclose($fp);
        return $expires > time();
    }
...
/* vivvo_cache support begin */
            @fwrite($fp,  sprintf('%016x', $this->_lifeTime));
/* vivvo_cache support end */
            if ($this->_readControl) {
