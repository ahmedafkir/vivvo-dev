        ...
        <div class="image_play">
            <a id="video_article_{article.get_id}" href="{CURRENT_CATEGORY.get_href}#play" class="play">
                <vte:if test="{article.image}">
                    <img src="{VIVVO_STATIC_URL}thumbnail.php?file={article.image}&amp;size=summary_medium" alt="{image_caption}" />
                    <vte:else>
                        <img src="{VIVVO_THEME}img/play_button_light.png" alt="{LNG_PLUGIN_VIDEO_BOX_PLAY}" title="{LNG_PLUGIN_VIDEO_BOX_PLAY}" />
                    </vte:else>
                </vte:if>
            </a>
        </div>
